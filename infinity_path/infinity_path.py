...
def control():
    ...
    while (time.time()- start) <20:
        ...
        elif ((time.time() - start) > 11) and ((time.time() - start) <= 16.7) :
                #curve
                pub_SteeringAngle.publish(-18.5)
                pub_cmd_vel.publish(0.23)
                pub_brakes.publish(0)     
        ...
        elif ((time.time() - start) >16,7) and ((time.time() - start) <=18.5):
                #straight 
                pub_SteeringAngle.publish(0)
                pub_cmd_vel.publish(.23)  


        elif ((time.time() - start) >18.6) and ((time.time() - start) <=19):
                #straight 
                pub_SteeringAngle.publish(0)
